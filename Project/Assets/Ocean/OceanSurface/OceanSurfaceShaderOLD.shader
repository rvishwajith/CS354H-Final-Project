Shader "Ocean/Ocean"
{
        ...
        SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            // Compute fresnal based on view direction. The more aligned the normal is with the view direction,
            // the higher the emission value should be,
            float fresnel = dot(worldNormal, viewDir);
            // Added by Rohith, originally fresnal was just set to 1 - fresnel
            float fresnel_neg = saturate(1 - fresnel);
            float fresnel_exp = pow5(fresnel_neg);
            o.Emission = lerp(color * (1 - fresnel_exp), 0, jacobian);
        }
        ENDCG
    }
        FallBack "Diffuse"
