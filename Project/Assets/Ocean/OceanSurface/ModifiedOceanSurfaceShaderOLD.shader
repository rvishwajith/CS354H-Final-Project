Shader "Ocean/ModifiedOceanSurface"
{
    Properties
    {
        ...
        SubShader
    {
        ...
        // Input values are computed here.
        void vert(inout appdata_full v, out Input o)
        {
            ...
            // Lowest detail: Add the largest wave cascades.
            displacement += tex2Dlod(_Displacement_c0, worldUV / LengthScale0) * lod_c0;

            // Update large wave bias after the first cascade for computing LODs.
            largeWavesBias = displacement.y;

            // Medium detail: Add the medium wave cascades.
            #if defined(MID) || defined(CLOSE)
                displacement += tex2Dlod(_Displacement_c1, worldUV / LengthScale1) * lod_c1;
            #endif

            // Highest detail: Add the smallest wave cascades.
            #if defined(CLOSE)
        ...
        float3 WorldToTangentNormal(Input IN, float3 normal) {
        ...
        float JacobianHighDetail(Input IN) {
            return tex2D(_Turbulence_c0, IN.worldUV / LengthScale0).x
                + tex2D(_Turbulence_c1, IN.worldUV / LengthScale1).x
                + tex2D(_Turbulence_c2, IN.worldUV / LengthScale2).x;
        }

        float JacobianMedDetail(Input IN) {
            return tex2D(_Turbulence_c0, IN.worldUV / LengthScale0).x
                + tex2D(_Turbulence_c1, IN.worldUV / LengthScale1).x;
        }

        float JacobianLowDetail(Input IN) {
            return tex2D(_Turbulence_c0, IN.worldUV / LengthScale0).x;
        }

        float Fresnel(float3 worldNormal, float3 viewDir) {
            // Compute fresnel based on view direction. The more aligned the normal is with the view direction,
            // the higher the emission value should be,
            float fresnel = dot(worldNormal, viewDir);
            // Added by Rohith, originally fresnel was just set to 1 - fresnel
            float fresnel_neg = saturate(1 - fresnel);
            float fresnel_exp = pow(fresnel_neg, 5);
            return fresnel_exp;
        }
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            o.Normal = WorldToTangentNormal(IN, worldNormal);
            
            // Depending on the LOD, sample the different turbulence values.
            #if defined(CLOSE)
                float jacobian = JacobianHighDetail(IN);
                jacobian = min(1, max(0, (-jacobian + _FoamBiasLOD2) * _FoamScale));
            #elif defined(MID)
                float jacobian = JacobianMedDetail(IN);
                jacobian = min(1, max(0, (-jacobian + _FoamBiasLOD1) * _FoamScale));
            #else
                float jacobian = JacobianLowDetail(IN);
            ...
            float depthDiff = max(0, backgroundDepth - surfaceDepth - 0.1);
            ...
            float ViewDotH = pow(saturate(dot(viewDir, -H)), 5) * 30 * _SSSStrength;
